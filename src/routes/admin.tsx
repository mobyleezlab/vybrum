...
import { ArrowLeft, Pencil, Plus, Trash2, Upload, Loader2, Lock } from "lucide-react";
...
function AdminPage() {
  ...
  if (loading || (user && check.isLoading)) {
    return <div className="grid min-h-dvh place-items-center bg-black text-white"><Loader2 className="h-6 w-6 animate-spin" /></div>;
  }
  ...
  return (
    <div className="min-h-dvh bg-black pb-20 text-white">
      <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-[#1a1a1a] bg-black/90 px-4 py-3 backdrop-blur">
        <Link to="/perfil" className="press grid h-9 w-9 place-items-center rounded-lg border border-[#2a2a2a] bg-[#1a1a1a]"><ArrowLeft className="h-4 w-4" /></Link>
        <h1 className="flex-1 text-base font-bold">Admin · Modelos</h1>
        <button
      ...
      <div className="px-4 py-4">
        {models.isLoading ? (
          ...
          <div className="mt-10 text-center text-sm text-[#888]">Nenhum modelo cadastrado. Clique em "Novo".</div>
        ) : (
...
function ModelEditor({ initial, onClose }: { initial: AdminModelInput; onClose: () => void }) {
  ...
  async function handleUpload(kind: "thumb" | "frente" | "costas", file: File) {
    if (!m.code) {
      toast.error("Defina o código primeiro.");
      return;
    }
    const max = kind === "thumb" ? 2_000_000 : 4_000_000;
    if (file.size > max) {
      toast.error(`Arquivo muito grande (máx ${Math.round(max / 1_000_000)}MB).`);
      return;
    }
    try {
    ...
    <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 sm:place-items-center" onClick={onClose}>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Código">
            <input
              value={m.code} onChange={(e) => update("code", e.target.value.toUpperCase())}
              disabled={!isNew}
              placeholder="VY010" maxLength={32}
              className="input"
            />
          </Field>
          <Field label="Nome">
            <input value={m.name} onChange={(e) => update("name", e.target.value)} maxLength={80} className="input" />
          </Field>
          <Field label="Categoria">
            <select value={m.category} onChange={(e) => update("category", e.target.value as AdminModelInput["category"])} className="input">
              {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
          </Field>
          <Field label="Esporte">
            <input value={m.sport} onChange={(e) => update("sport", e.target.value)} className="input" />
          </Field>
          <Field label="Custo desbloqueio (créditos)">
            <input type="number" value={m.unlock_cost ?? 0} onChange={(e) => update("unlock_cost", Number(e.target.value))} className="input" />
          </Field>
          <Field label="Ordem">
            <input type="number" value={m.sort_order} onChange={(e) => update("sort_order", Number(e.target.value))} className="input" />
          </Field>
          <Field label="Drop (nome opcional)">
            <input value={m.drop_name ?? ""} onChange={(e) => update("drop_name", e.target.value || null)} className="input" />
          </Field>
          <Field label="Disponível até">
            <input type="date" value={m.available_until?.slice(0, 10) ?? ""} onChange={(e) => update("available_until", e.target.value ? new Date(e.target.value).toISOString() : null)} className="input" />
          </Field>
          <Field label="Limitado">
            <Toggle value={!!m.is_limited} onChange={(v) => update("is_limited", v)} />
          </Field>
          <Field label="Premium">
            <Toggle value={!!m.is_premium} onChange={(v) => update("is_premium", v)} />
          </Field>
        </div>
        ...
        <div className="mt-5 flex items-center justify-between gap-2">
          {!isNew ? (
            <button
              onClick={() => { if (confirm(`Remover "${m.name}"?`)) del.mutate(); }}
          ...
          <button
            onClick={() => save.mutate()}
            disabled={save.isPending || !m.code || !m.name}
      ...
      <style>{`
        .input { width:100%; height:40px; border-radius:8px; border:1px solid #2a2a2a; background:#161616; color:#fff; padding:0 10px; font-size:13px; }
        .input:focus { outline:none; border-color:#68ed00; }
      `}</style>
    </div>
  );
}
    ...
    <div className="rounded-xl border border-[#2a2a2a] bg-[#161616] p-2">
      ...
      <div className="grid aspect-square place-items-center rounded-md bg-[#0f0f0f]">
        {url ? (
          ...
          <span className="text-[10px] text-[#444]">sem arquivo</span>
        )}
      </div>
      <input
